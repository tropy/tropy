...
WITH
  ...
  SELECT l.list_id, json_group_array(c.list_id) AS children
    FROM lists l LEFT OUTER JOIN children c
    WHERE l.list_id = 0
    GROUP BY l.list_id;
...
SELECT l1.list_id, l1.name, group_concat(l2.list_id || ':' || l2.position) AS children
  FROM lists l1 LEFT OUTER JOIN lists l2 ON l2.parent_list_id = l1.list_id
  WHERE l1.parent_list_id = 0
  GROUP BY l1.list_id
  ORDER BY l1.position ASC;
...
WITH
  children(parent_list_id, list_id) AS (
    SELECT parent_list_id, list_id FROM lists ORDER BY parent_list_id, position ASC
  )
